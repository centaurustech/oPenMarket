...
<script type="text/javascript">
    ...
    function msg(nam, price, id, cant)
    {
    	...
    	if(window.location.href.indexOf("fact.php?param=100")>0)
    	{
    		...
    		$('#Jtabla').append('<tr class="odd gradeB"><td>' + nam + '</td><td class="center">' + cant + '</td><td class="center">15.1%</td><td class="center">12.5%</td><td>300000</td><td>600000</td></tr>');
...
function consulta(typ)
{
		...
		success: 
			...
			if(typ==2)
			{
				if(data!="")
				{
					...
					{
						$('#infobox').html('<img src="img/warning.png">  Este producto no se encuentra en nuestro stock, Puede solicitarnos via e-mail');
						$('#infobox').removeClass("oculto successbox");
						$('#infobox').addClass("warningbox");
						$('#infobox').hide();
						$('#infobox').fadeIn(2000);
	...
	{		
		...
		{			
			...
			$('#btnLogin').html('<span>Cerrar Sesión</span>');
