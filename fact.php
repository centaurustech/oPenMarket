...
<?php			
	...
	echo '<div class="warningbox">Aun no te haz logeado en tu cuenta, o aun no estas registrado.. Necesitas estar registrado para efectuar compras</div>';		
...
<table>
	<thead>
		...
		<tr>
			...
			<td align="left" width="1%"><b>Fecha: </b><?php echo date("d/m/Y - h:m:s"); ?> </td>			
		</tr>		
...
$dat=$cnx->createCommand("select ventas.dirEnv,ventas.paisEnv,ventas.city from ventas where ventas.iduser='" . $_COOKIE['pass'] . "'  order by ventas.transid DESC")->query();
$dat->bindColumn(1, $tmp1);
$dat->bindColumn(2, $tmp2);	
$dat->bindColumn(3, $tmp3);
$dat->bindColumn(4, $tmp4);
$dat->read();
...
<div id="container" style="text-align:center;">
    ...
    , donde podras consultar mas detallamente precios, IVA, descuentos y más sobre tus productos a adquirir <br><br>
    
    <?php 

		if(isset($_GET['param']) && $_GET['param']>0)
		echo "Si desea un presupuesto mas personalizado <a href='#' onclick='hahahaha()' class='osx'>Haz click aqui</a>";

			else
			echo 'la casilla <b>Dirección</b> sera relativa a la ultima compra efectuada</p>';
